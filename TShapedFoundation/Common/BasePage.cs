...
namespace TShapedFoundation.Common
{
    class BasePage
    {
        ...
        public String GetElementAttribute(By byLocator, string attribute)
        {
            return this.FindElement(byLocator).GetAttribute(attribute);
        }
        ...
        public void ClickChildElementInHoveredParentElement(By parentLocator, By childLocator)
        {
            var action = new Actions(driver);
            var we = FindElement(parentLocator);
            action.MoveToElement(we).MoveToElement(FindElement(childLocator)).Click().Build().Perform();
        }

        public void SelectItemForDropdownByText(By byLocator, string value)
        {
            SelectElement dropdown = new SelectElement(FindElement(byLocator));
